...
pub fn create_system_prompt(config: &Config, prompt_type: PromptType) -> anyhow::Result<String> {
    match prompt_type {
        PromptType::CommitMessage => create_commit_system_prompt(config),
        PromptType::PullRequest => create_pr_system_prompt(config),
        PromptType::Completion => create_completion_system_prompt(config),
        PromptType::Changelog => Ok(create_changelog_system_prompt(config)),
        PromptType::ReleaseNotes => Ok(create_release_notes_system_prompt(config)),
    }
}
...
pub fn create_user_prompt(
    ...
    match prompt_type {
        PromptType::CommitMessage => {
            create_commit_user_prompt(context, detail_level.unwrap_or(DetailLevel::Standard))
        }
        PromptType::PullRequest => {
            let commit_messages = match extra {
                Some(PromptExtra::CommitMessages(v)) => v,
                _ => Vec::new(),
            };
            create_pr_user_prompt(context, &commit_messages)
        }
        PromptType::Completion => {
            let (prefix, context_ratio) = match extra {
                Some(PromptExtra::Completion {
                    prefix,
                    context_ratio,
                }) => (prefix, context_ratio),
                _ => (String::new(), 0.5),
            };
            create_completion_user_prompt(context, &prefix, context_ratio)
        }
        PromptType::Changelog | PromptType::ReleaseNotes => String::new(),
    }
}

pub enum PromptExtra {
    CommitMessages(Vec<String>),
    Completion { prefix: String, context_ratio: f32 },
}

pub fn create_pr_system_prompt(config: &Config) -> anyhow::Result<String> {
    let pr_schema = schemars::schema_for!(super::super::commit::types::GeneratedPullRequest);
...
fn create_commit_system_prompt(config: &Config) -> anyhow::Result<String> {
    let commit_schema = schemars::schema_for!(super::super::commit::types::GeneratedMessage);
...
fn create_completion_system_prompt(config: &Config) -> anyhow::Result<String> {
    let completion_schema = schemars::schema_for!(super::super::commit::types::GeneratedMessage);
...
fn create_changelog_system_prompt(config: &Config) -> String {
    let combined_instructions = get_combined_instructions(config);
    format!(
        "# PERSONA\n\
         You are a Release Notes Maintainer. You create detailed, well-structured changelogs \
         that help developers understand what changed and why.\n\
         \n\
         # TASK\n\
...
fn create_release_notes_system_prompt(config: &Config) -> String {
    let combined_instructions = get_combined_instructions(config);
    format!(
        "# PERSONA\n\
         You are a Release Notes Maintainer. You create detailed, well-structured release notes \
         that help users understand what changed and how to upgrade.\n\
         \n\
         # TASK\n\
         Generate comprehensive release notes with highlights, breaking changes, and upgrade notes.\n\
         \n\
         # USER INSTRUCTIONS\n\
         {}\n",
        combined_instructions
...
fn create_commit_user_prompt(context: &CommitContext, detail_level: DetailLevel) -> String {
        ...
        let content_section = content_files
            ...
            .map(|file| {
                let change_indicator = match file.change_type {
                    ...
                    ChangeType::Modified => "\u{270F}",
                    ChangeType::Renamed { .. } => "\u{1F69A}",
                    ChangeType::Copied { .. } => "\u{1F4CB}",
                };
